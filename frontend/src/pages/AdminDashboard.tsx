...
const AdminDashboard: React.FC = () => {
  ...
  const fetchPosts = async () => {
    try {
      const [allPosts, publishedPosts, tagsResponse, usersResponse] = await Promise.all([
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {users.map((user) => (
                        <tr key={user.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.user_type === 'admin' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
